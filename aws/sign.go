...
func EnvAuth() (auth Auth, err error) {
	if auth.AccessKey = os.Getenv("AWS_ACCESS_KEY"); auth.AccessKey == "" {
		return errors.New("AWS_ACCESS_KEY not found in environment")
	}
	if auth.SecretKey = os.Getenv("AWS_SECRET_KEY"); auth.SecretKey == "" {
		return errors.New("AWS_SECRET_KEY not found in environment")
	}
	return
}
